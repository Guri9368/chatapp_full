  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      Cookies.set("token", data.token, {
        expires: 15,
        secure: false, // because we host it aws and if we use https then it will be secure but there we use http if we make it true then we apply api gateway for https
        path: "/",
      });
      setOtp(["", "", "", "", "", ""]);
